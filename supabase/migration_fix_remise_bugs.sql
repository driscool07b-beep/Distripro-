-- Migration : corrige deux bugs découverts en testant le seuil de remise
-- 1) ventes.remise_montant n'avait jamais été réellement créée (migration
--    précédente jamais exécutée) — creer_vente échouait à l'insertion.
-- 2) Le message d'erreur de dépassement de seuil utilisait une syntaxe de
--    formatage (%.1f) non supportée par PL/pgSQL RAISE, produisant un texte
--    illisible. Corrigé en construisant le texte avant de l'afficher.
-- À exécuter dans l'éditeur SQL de Supabase.

alter table ventes add column if not exists remise_montant numeric(14,2) not null default 0;
